...
# Function to install pnpm via corepack (bundled with Node.js >= 16.10)
function Enable-Pnpm {
	try {
		corepack --version | Out-Null
	} catch {
		Write-Output "corepack tidak ditemukan. Install manual: npm install -g pnpm"
		throw
	}
	corepack enable
...
# Ensure pnpm is available (uses corepack, no separate install)
try {
	pnpm --version | Out-Null
} catch {
	Write-Output "pnpm belum tersedia, mengaktifkan via corepack..."
	Enable-Pnpm
	Refresh-Env-Vars
}
...
if (-not (Test-Path -Path .env)) {
	if (Test-Path -Path .env.example) {
		...
		throw ".env.example tidak ditemukan. Buat .env secara manual."
	}
}
